...
//    NSURL *baseURL = [NSURL URLWithString:baseUrl];
    ...
    if (DeviceToken && DeviceToken.length > 0) {
     ...
     [manger.requestSerializer setValue:@"4100" forHTTPHeaderField:@"version"];
...
+ (NSURLSessionDataTask *)requestMethod:(GXHttpRequestMethod)requestMethod
    ...
    if([baseUrl rangeOfString:@"http"].location==NSNotFound)//_roaldSearchText
    {
        ...
        httpPrefix=[[self getHttpDic] objectForKey:UrlString];
        
        if (httpPrefix==nil) {
            httpPrefix=@"https";
        }
        
        UrlString=[NSString stringWithFormat:@"%@://%@%@",httpPrefix,baseUrl,UrlString];
    }else{
    ...
    YYCache *cacheTool = [YYCache cacheWithName:GXHttpRequestCache];
...
+ (NSURLSessionDataTask *)requestMethod:(GXHttpRequestMethod)type
    ...
    // 登录后增加token
    NSString *UserToken = [GXUserInfoTool getUserTocken];
...
+(void)post:(NSString *)url image:(NSData *)image name:(NSString *)name success:(void (^)(id))success failure:(void (^)(NSError *))failure {
    GXHttpTool *manger = [GXHttpTool shareHttpTool];
    [manger.requestSerializer willChangeValueForKey:@"timeoutInterval"];
    ...
    if([baseUrl rangeOfString:@"http"].location==NSNotFound)//_roaldSearchText
    {
        ...
        httpPrefix=[[self getHttpDic] objectForKey:url];
        
        if (httpPrefix==nil) {
            httpPrefix=@"https";
        }
        
        url=[NSString stringWithFormat:@"%@://%@%@",httpPrefix,baseUrl,url];
    }else{
...
+ (void)registerDevice{
    ...
    if([baseUrl rangeOfString:@"http"].location==NSNotFound)//_roaldSearchText
    {
      DeviceRegisteUrl=[NSURL URLWithString:[NSString stringWithFormat:@"https://%@%@",baseUrl,GXUrl_DeviceRegister]];
    }else{
    
      DeviceRegisteUrl=[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",baseUrl,GXUrl_DeviceRegister]];
