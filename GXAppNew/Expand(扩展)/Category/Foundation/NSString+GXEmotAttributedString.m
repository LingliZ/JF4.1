...
+ (NSAttributedString *)dealContentText:(NSString *)questionContent{
    ...
    while (results.count > 0) {
        ...
        NSAttributedString *attrStr = [NSAttributedString yy_attachmentStringWithContent:imageView contentMode:UIViewContentModeScaleAspectFit attachmentSize:CGSizeMake(emotionFont.lineHeight, emotionFont.lineHeight) alignToFont:emotionFont alignment:YYTextVerticalAlignmentCenter];
        if (!image) {
...
+ (NSAttributedString *)dealContentText:(NSString *)questionContent withView: (UIView *)view{
    ...
    [httpResultArray enumerateObjectsUsingBlock:^(NSTextCheckingResult * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        ...
        YYTextHighlight *highlight = [[YYTextHighlight alloc] init];
        YYTextBorder *textBorder = [YYTextBorder borderWithFillColor:[UIColor colorWithWhite:0.000 alpha:0.220] cornerRadius:0];
        highlight.tapAction = ^void(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect){
            NSString *urlStr = [text.string substringWithRange:range];
            SFSafariViewController *safari = [[SFSafariViewController alloc] initWithURL:[NSURL URLWithString:urlStr]];
            [view.viewController presentViewController:safari animated:true completion:nil];
        };
        [highlight setColor:[UIColor colorWithRed:0.093 green:0.492 blue:1.000 alpha:1.000]];
        [highlight setBackgroundBorder:textBorder];
        [attrM addAttributes:@{NSForegroundColorAttributeName :[UIColor blueColor]} range:highlightRange];
