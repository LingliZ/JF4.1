...
- (void)insertEmotion:(GXEmotion *)emotion{
    ...
    [textAttr addAttribute:NSFontAttributeName value:emotionFont range:NSMakeRange(selectedRange.location, 1)];
...
- (BOOL)isFullEmot{
    ...
    if ([content hasPrefix:@"@"]) {
        NSRange subRange = [content rangeOfString:@" "];
        range = NSMakeRange(subRange.location + subRange.length, attr.length - (subRange.location + subRange.length));
    }
    [attr enumerateAttributesInRange:range options:NSAttributedStringEnumerationLongestEffectiveRangeNotRequired usingBlock:^(NSDictionary<NSString *,id> * _Nonnull attrs, NSRange range, BOOL * _Nonnull stop) {
        GXTextAttachment *textAttach = attrs[@"NSAttachment"];
        if (!textAttach) {
            _isFullEmot = false;
        }
