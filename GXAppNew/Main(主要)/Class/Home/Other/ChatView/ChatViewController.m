...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [AppDelegate updateAppBadgeNumber];
    
    MessageModel*model=[[MessageModel alloc]init];
    model.isSender=NO;
    model.content=[GXUserdefult objectForKey:HomeSeviceInfo];
    model.type=1;
...
- (void)viewDidLoad
{
    ...
    [self addItemForAddCount];
    
    NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:@"experience-kefu#sandbox" forKey:kAppKey];
//    [[EMIMHelper defaultHelper] setCname:@"u1423191635866"];
 //   [[EMIMHelper defaultHelper] setCname:@"u1423191635866"];
    [[EMIMHelper defaultHelper] setCname:EaseMobCusterKey];
    [[EMIMHelper defaultHelper] setTenantId:@"26512"];
    [[EMIMHelper defaultHelper] setProjectId:@"184562"];
...
- (void)RegisterEaseMob {
    ...
    [self.tableView showLoadingWithTitle:@"正在连接客服"];
...
#pragma mark--导航栏右边按钮
-(void)addItemForAddCount
{
//    UIBarButtonItem*rightButtonItem=[[UIBarButtonItem alloc]initWithTitle:@"开户" style:UIBarButtonItemStylePlain target:self action:@selector(addCount)];
//    [rightButtonItem setTitleTextAttributes:@{NSFontAttributeName:GXFONT_PingFangSC_Light(GXFitFontSize14),NSForegroundColorAttributeName:UIColorFromRGB(0xFFFFFF)} forState:UIControlStateNormal];
//    self.navigationItem.rightBarButtonItem=rightButtonItem;
    UIButton*btn_call=[UIButton buttonWithType:UIButtonTypeCustom];
    [btn_call addTarget:self action:@selector(btnClick_call) forControlEvents:UIControlEventTouchUpInside];
    [btn_call setImage:[UIImage imageNamed:@"mine_call"] forState:UIControlStateNormal];
    [btn_call sizeToFit];
    
    UIBarButtonItem*item_call=[[UIBarButtonItem alloc]initWithCustomView:btn_call];
    self.navigationItem.rightBarButtonItem=item_call;
...
- (void)setupBarButtonItem
{
    ...
    [backButton setImage:[UIImage imageNamed:@"mine_back"] forState:UIControlStateNormal];
        ...
        if ([obj isKindOfClass:[NSString class]]) {
            ...
            if (cell == nil) {
                ...
                if (model.isSender) {
                    ...
                    NSString *headImageName = [GXUserInfoTool isLogin] ? [GXUserInfoTool getUserHeadImageName] : UserDefineHeadImage;
                    [cell.headImageView sd_setImageWithURL:[NSURL URLWithString:headImageName] placeholderImage:[UIImage imageNamed:headImageName]];
                }
                else{
                    cell.headImageView.image = [UIImage imageNamed:onlineCutemerHeadImage];
                }
            }
...
#pragma mark - UIResponder actions--满意度调查、查看大图、读取语音、读取视频
...
#pragma mark--浮动菜单（长按某条消息实现删除、拷贝）
...
{
    GXLog(@"------%d",[[EaseMob sharedInstance].chatManager isLoggedIn]);
 ...
 //willSendMessage:error:和didSendMessage:error:
...
- (NSDictionary*)getUserInfoAttribute
{
    ...
    BOOL isLogin = [GXUserInfoTool isLogin];
    NSString *realName = isLogin ? [GXUserInfoTool getUserReallyName]:@"无";
    NSString *nickName = isLogin ? [GXUserInfoTool getUserNickName]:@"游客";
    NSString *phone = isLogin ? [GXUserInfoTool getPhoneNum]:@"无";
    
    
    [visitor setObject:realName forKey:@"trueName"];
    [visitor setObject:phone forKey:@"phone"];
    [visitor setObject:nickName forKey:@"userNickname"];
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    //[controller.gxTabBar hideBadgeOnItemIndex:3];
}
