...
+(instancetype)modelWithDictionary:(NSDictionary *)dictionary{
    if ([dictionary[@"type"] isEqualToString:@"data"]) {
        ...
        CGFloat dataHeight = [GXAdaptiveHeightTool lableHeightWithText:dataModel.name font:[UIFont systemFontOfSize:16] Width:30];
        dataModel.cellHeight = dataHeight + 85;

        return dataModel;
    }else{
        GXCalendarEventModel *eventModel = [GXCalendarEventModel new];
        [eventModel setValuesForKeysWithDictionary:dictionary];
        CGFloat eventHeight = [GXAdaptiveHeightTool lableHeightWithText:[NSString stringWithFormat:@"【事件】%@",eventModel.event] font:[UIFont systemFontOfSize:16] Width:30];
        eventModel.cellHeight = eventHeight + 85;
