...
@interface GXCalendarPageView()<NSCacheDelegate>

@property (nonatomic, strong)NSCache *limitControllerCaches;

@end


@implementation GXCalendarPageView

{
    
    GXCalendarBaseView *pagerView;
    NSArray *myArray;
    NSArray *classArray;
    NSArray *colorArray;
...
-(void)setCustomBottomLinePer:(CGFloat)customBottomLinePer{
    ...
    pagerView.customBottomLinePer = _customBottomLinePer;
}
...
- (NSCache *)limitControllerCache {
    if (!_limitControllerCaches) {
        _limitControllerCaches = [NSCache new];
        _limitControllerCaches.delegate = self;
    }
    /**< Set max of controller's number   **/
    _limitControllerCaches.countLimit = 5;
    return _limitControllerCaches;
}
...
- (void)createPagerView:(NSArray *)titles WithVCs:(NSArray *)childVCs WithColors:(NSArray *)colors {
    ...
    if (titles.count > 0 && childVCs.count > 0 && titleFontSize <= 0) {
        pagerView = [[GXCalendarBaseView alloc] initWithFrame:CGRectMake(0, 0, FUll_VIEW_WIDTH, FUll_CONTENT_HEIGHT) WithSelectColor:_selectColor WithUnselectorColor:_unselectColor WithUnderLineColor:_underlineColor WithtopTabColor:_topTabColor WithTopTabType:pagerStyle WithNinaDefaultPageIndex: ninaDefaultIndex];
        ...
        pagerView.scrollView.contentOffset = CGPointMake(FUll_VIEW_WIDTH * ninaDefaultIndex, 0);
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    if ([keyPath isEqualToString:kObserverPage]) {
        ...
        if (myArray.count > 7) {
            ...
            if (page >= 3) {
                if (page <= myArray.count - 4) {
                    topTabOffsetX = (page - 3) * More5LineWidth;
                }else {
                    if (page == myArray.count - 3) {
                        topTabOffsetX = (page - 4) * More5LineWidth;
                    }else if(page == myArray.count - 2){
                        topTabOffsetX = (page - 5) * More5LineWidth;
                    }else{
                        topTabOffsetX = (page - 6) * More5LineWidth;
                    }
                }
            }else{
                if (page == 1 || page == 2){
...
- (void)dealloc {
    [pagerView removeObserver:self forKeyPath:kObserverPage];
}
