...
- (void)loadOneLineUi{
    ...
    {
        GXActionButton *button = [GXActionButton buttonWithType:UIButtonTypeCustom];
        button.frame = CGRectMake(_backGroundView.bounds.size.width/4 * i, _LXActionSheetHeight + 18 , _backGroundView.bounds.size.width/4, 86);
...
-(UIScrollView *)scrollowView{
    if (_scrollowView == nil) {
        ...
        _scrollowView.contentSize = CGSizeMake([UIScreen mainScreen].bounds.size.width, 0);
        _scrollowView.showsVerticalScrollIndicator = NO;
        _scrollowView.showsHorizontalScrollIndicator = NO;
//        _scrollowView.alwaysBounceHorizontal = NO;
...
- (UIButton *)cancelBtn
{
    if (_cancelBtn == nil) {
        ...
        else if (_showtype == ShowTypeIsActionSheetStyle)
        {
            ...
            _cancelBtn.titleLabel.font = GXFONT_PingFangSC_Regular(GXFitFontSize17);
