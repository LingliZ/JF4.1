...
- (void)updateProgressLayer {
    ...
    // 右边
    if (self.type == 1) {
        _progressLayer.anchorPoint = CGPointMake(1, 0);
        _progressLayer.frame = CGRectMake(self.bounds.size.width, 0, self.bounds.size.width * -(self.progress), self.bounds.size.height);
    }
